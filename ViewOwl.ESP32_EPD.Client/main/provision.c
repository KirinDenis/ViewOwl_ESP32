/**
 * @file provision.c
 * @brief Serial provisioning for the ESP32-S3 e-paper client (USB-Serial-JTAG).
 *
 * EPD port of ../ViewOwl.ESP32_C3.Client/main/provision.c. The line protocol and
 * NVS writes are identical (must match BurnModal.jsx); only the on-screen
 * diagnostics are dropped: an e-paper panel refreshes far too slowly to show a
 * per-byte heartbeat, so progress is reported over the serial log instead.
 */
    ...
    while (elapsed_ms < timeout_ms) {
        ...
        if ((char)byte == '\n') {
            ...
            if (strncmp(line, PREFIX_TOKEN, PREFIX_TOKEN_LEN) == 0) {
                ...
                /* Pin config the dashboard sends before WiFi - the e-paper board
                 * has fixed pins, so ignore it (the browser reads no response). */
                continue;
