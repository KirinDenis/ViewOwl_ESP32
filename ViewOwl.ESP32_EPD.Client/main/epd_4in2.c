...
/* Stream one 4-gray plane for a w x h region. Region image is 2-bit packed,
 * MSB-first, row stride = ceil(w/4) bytes. Output is ceil(w/8) bytes per row. */
static void epd_send_gray_plane_region(const uint8_t *image, uint16_t w, uint16_t h, bool plane24)
{
    uint16_t in_stride  = (w + 3) / 4;
    uint16_t out_bytes  = (w + 7) / 8;
    for (uint16_t row = 0; row < h; row++) {
        const uint8_t *src = image + row * in_stride;
        uint16_t px = 0;
        for (uint16_t ob = 0; ob < out_bytes; ob++) {
            uint8_t out = 0;
            for (int bit = 0; bit < 8; bit++) {
                uint8_t code = 0;
                if (px < w) {
                    code = (src[px >> 2] >> (6 - (px & 3) * 2)) & 0x03;
                }
                uint8_t b = plane24 ? ((code == 3 || code == 1) ? 1 : 0)   /* white & dark-grey */
                                    : ((code == 3 || code == 2) ? 1 : 0);  /* white & light-grey */
                out = (uint8_t)((out << 1) | b);
                px++;
            }
            epd_wr_data(out);
...
void epd_display_part_4gray(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint8_t *image)
{
    ...
    epd_set_cursor(x, y);
    epd_wr_reg(0x24);
    epd_send_gray_plane_region(image, w, h, true);

    epd_set_window(x, y, x + w - 1, y + h - 1);
    epd_set_cursor(x, y);
    epd_wr_reg(0x26);
    epd_send_gray_plane_region(image, w, h, false);

    epd_wr_reg(0x22);                       /* same grey LUT update as full 4-gray */
    epd_wr_data(0xCF);
    epd_wr_reg(0x20);
    epd_wait_busy();
    ESP_LOGI(TAG, "4-gray PARTIAL (%u,%u %ux%u) done", x, y, w, h);
}
